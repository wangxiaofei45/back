	...
	export default {
		...
		methods: {
			ajaxjson() {
				...
				this.$http.get(apiUrl + '/member/update?id=' + this.$route.params.id).then((response) => {
			...
			submit() {
				...
				let apiUrl = this.apiUrl;
				this.$http.post(apiUrl + '/member/update?id='+ this.$route.params.id, obj, {
					emulateJSON: true
				}).then((response) => {
...
//				let apiUrl = this.apiUrl;
//				console.log(obj);
//				this.$http.post(apiUrl + '/member/update', objs, {
//						headers: {
